...
export default function BottomBar({ selectedClient }: Props) {
  ...
  useEffect(() => {
    ...
    const check = () => {
      window.api.callBackend('/api/status')
        .then((result: any) => {
          if (ignoreRef.current) return
          setStatusOk(!(result && result.error))
        })
        .catch(() => {
          if (ignoreRef.current) return
          if (attempt < maxAttempts) {
            attempt++
            timer = setTimeout(check, Math.min(500 * Math.pow(1.5, attempt - 1), 5000))
          } else {
            setStatusOk(false)
          }
        })
    ...
    check()
    return () => { ignoreRef.current = true; clearTimeout(timer) }
  }, [])
