...
class TestPickClient:
    @patch("rs3tk.cli.detect_client")
    @patch("rs3tk.cli.get_client_keys")
    @patch("rs3tk.cli.Prompt.ask")
...
class TestAuthLogin:
    ...
    @patch("rs3tk.cli.do_login")
    def test_system_browser_flag(self, mock_do: MagicMock, runner: CliRunner) -> None:
        mock_do.return_value = ("u", 1)

        result = runner.invoke(main, ["auth", "login", "-b"])

        assert result.exit_code == 0
        mock_do.assert_called_once_with(system_browser=True)

    @patch("rs3tk.cli.do_login", side_effect=AppError("login failed"))
...
class TestAccountsGroup:
    def test_no_subcommand_invokes_list(self, runner: CliRunner) -> None:
        with patch("rs3tk.cli.get_all_characters", return_value=[]):
    ...
    def test_list_with_characters(self, runner: CliRunner) -> None:
        ...
        with (
            patch("rs3tk.cli.get_all_characters", return_value=chars),
            patch("rs3tk.cli.load_settings", return_value=Settings()),
        ):
            result = runner.invoke(main, ["accounts", "list"])
        assert result.exit_code == 0
        assert "Alice" in result.output
...
class TestAccountsSetDefault:
    @patch("rs3tk.cli.set_default_character")
    @patch("rs3tk.cli.get_all_characters")
...
class TestAccountsUnsetDefault:
    @patch("rs3tk.cli.unset_default_character")
...
class TestClientsGroup:
    def test_no_subcommand_invokes_list(self, runner: CliRunner) -> None:
        with patch("rs3tk.cli.get_client_info", return_value=[]):
            result = runner.invoke(main, ["clients"])
        assert result.exit_code == 0


class TestClientsList:
    @patch("rs3tk.cli.get_client_info")
...
class TestClientsInstall:
    @patch("rs3tk.cli.do_autoinstall")
    def test_invokes(self, mock_do: MagicMock, runner: CliRunner) -> None:
...
class TestPlay:
    ...
    def test_no_client_arg_invokes_picker(
        ...
        with patch("rs3tk.cli.launch_game"):
...
class TestStatus:
    @patch("rs3tk.cli.check_game_status")
...
class TestNews:
    ...
    def test_explicit_game_overrides_default(self, mock_news: MagicMock, runner: CliRunner) -> None:
        with patch("rs3tk.cli.load_settings", return_value=Settings(default_game="osrs")):
...
class TestConfigGroup:
    def test_no_subcommand_shows_settings(self, runner: CliRunner) -> None:
        with patch("rs3tk.cli.get_config", return_value=Settings()):
...
# ── gui (PySide6 launcher) ──────────────────────────────────────────────────


class TestGuiCommand:
    def test_missing_pyside6(self, runner: CliRunner) -> None:
        # We can't really uninstall PySide6 in CI, but we can simulate the
        # ImportError by patching the import inside the gui command body.
        with patch.dict("sys.modules", {"PySide6": None}):
            result = runner.invoke(main, ["gui"])
        # Either PySide6 is missing -> ClickException, or it's installed and
        # we'd enter the event loop (which would hang). The test only asserts
        # behavior when the import fails.
