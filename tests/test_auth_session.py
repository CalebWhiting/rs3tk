...
from rs3tk.auth import session as session_module
from rs3tk.auth.session import (
    _ensure_session,
    _store_tokens,
    ensure_valid_token,
    get_session,
...
from rs3tk.jagex_api import Tokens, UserProfile
...
class TestStoreAndLoad:
    @patch("rs3tk.auth.session.set_account_token")
    ...
    @patch("rs3tk.auth.session.get_account_token")
...
class TestLogoutAccount:
    @patch("rs3tk.auth.session.clear_account")
...
class TestEnsureSession:
    @patch("rs3tk.auth.session.get_account_token")
    @patch("rs3tk.auth.session.ensure_valid_token", new_callable=AsyncMock)
    ...
    @patch("rs3tk.auth.session.create_session", new_callable=AsyncMock)
    @patch("rs3tk.auth.session.set_account_token")
    @patch("rs3tk.auth.session.get_account_token")
    @patch("rs3tk.auth.session.ensure_valid_token", new_callable=AsyncMock)
...
class TestGetSession:
    @patch("rs3tk.auth.session.get_profile", new_callable=AsyncMock)
    @patch("rs3tk.auth.session._ensure_session", new_callable=AsyncMock)
