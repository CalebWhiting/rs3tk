...
class TestDoLogout:
    @patch("rs3tk_core.app.save_settings")
    @patch("rs3tk_core.app.load_settings")
    @patch("rs3tk_core.app.logout_account")
...
class TestGetSessionAndProfile:
    ...
    def test_no_username_uses_first_account(self, mock_load: MagicMock) -> None:
        ...
        with patch("rs3tk_core.app.run_sync", return_value=("sid", _user_profile())) as mock_run:
...
class TestGetCharactersResult:
    ...
    @patch("rs3tk_core.app.get_session")
    @patch("rs3tk_core.app.load_settings")
...
class TestGetAccountForCharacter:
    @patch("rs3tk_core.app.get_all_characters")
...
class TestGetGameClient:
    @patch("rs3tk_core.app.detect_client")
...
class TestCheckGameStatus:
    @patch("rs3tk_core.app.check_status")
    ...
    def test_wraps_game_error_in_app_error(self, mock_check: MagicMock) -> None:
        from rs3tk_core.game import GameError
...
class TestGetNews:
    @patch("rs3tk_core.app.fetch_news")
    @patch("rs3tk_core.app.load_settings")
...
class TestGetClientInfo:
    @patch("rs3tk_core.app.get_client_keys")
    @patch("rs3tk_core.app.detect_client")
...
class TestDoAutoinstall:
    @patch("rs3tk_core.app.update_client_config")
    @patch("rs3tk_core.app.shutil.rmtree")
    @patch("rs3tk_core.app.CLIENTS_DIR")
    ...
    @patch("rs3tk_core.app.update_client_config")
    @patch("rs3tk_core.app.subprocess.run")
    @patch("rs3tk_core.app.install_client")
    ...
    def test_install_raises_on_install_error(self, mock_install: MagicMock) -> None:
        from rs3tk_core.install import InstallError
...
class TestLaunchGame:
    ...
    def test_no_character_path(self, mock_ggc: MagicMock) -> None:
        client = MagicMock()
        client.name = "RuneLite"
    ...
    @patch("rs3tk_core.app.get_session_and_profile")
    @patch("rs3tk_core.app.get_account_for_character")
    @patch("rs3tk_core.app.save_settings")
    @patch("rs3tk_core.app.load_settings")
    @patch("rs3tk_core.app.get_game_client")
    ...
    @patch("rs3tk_core.app.get_game_client")
    def test_wraps_launch_errors(self, mock_ggc: MagicMock) -> None:
        client = MagicMock()
        client.name = "RuneLite"
